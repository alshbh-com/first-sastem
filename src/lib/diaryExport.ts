...
export function exportDiaryToPDF(
  ...
  const financialSection = printFinancial ? `
    ...
    <div class="summary">
      ...
      <div>الواصل نقدي: <strong>${totalCash}</strong> | الرصيد: <strong>${balanceNum}</strong> | مستحق سابق: <strong>${previousDueNum}</strong></div>
  ...
  const orangeSection = printOrange ? `
    ...
    <div class="summary">
      ...
      ${extraDue > 0 ? `<div>مستحق إضافي: <strong>${extraDue}</strong>${extraDueReason ? ` (${extraDueReason})` : ''}</div>` : ''}
