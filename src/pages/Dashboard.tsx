...
import {
  ...
  CircleDot, Calculator, Contact, Clock, CheckCircle2, XCircle, AlertTriangle
} from 'lucide-react';
...
export default function Dashboard() {
  ...
  const [stats, setStats] = useState({ total: 0, open: 0, delivered: 0, returned: 0, todayCount: 0, todayRevenue: 0 });
  ...
  const loadStats = async () => {
    ...
    const [allRes, statusRes] = await Promise.all([
      supabase.from('orders').select('id, is_closed, status_id, price, delivery_price, created_at'),
      supabase.from('order_statuses').select('id, name'),
    ]);
    const all = allRes.data || [];
    const sts = statusRes.data || [];
    const deliveredIds = sts.filter(s => s.name?.includes('تسليم') || s.name?.includes('مسلم')).map(s => s.id);
    const returnedIds = sts.filter(s => s.name?.includes('مرتجع') || s.name?.includes('رفض')).map(s => s.id);
    ...
    setStats({
      ...
      todayRevenue: todayOrders.reduce((s, o) => s + Number(o.price) + Number(o.delivery_price), 0),
    });
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-3 grid-cols-2 sm:grid-cols-3 lg:grid-cols-6">
        ...
        <Card className="bg-card border-border"><CardContent className="p-3 text-center">
          ...
          <p className="text-lg font-bold">{stats.todayRevenue.toLocaleString()}</p>
        </CardContent></Card>
      </div>
